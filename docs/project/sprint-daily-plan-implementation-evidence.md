...
| SPD-09 Simplified Member-capacity presentation | AC35–AC39, AC76 | `SprintTaskReview.tsx`: removes `SummaryGrid`/`DateWindowControls`, renders one Capacity row per Member, formats dates, hides WBS/aggregate Task metadata, wraps Task names at `50ch`, and derives a whole-column `No capacity` marker directly from `member.dailyCapacity` | `SprintTaskReview.test.tsx` — `renders Member daily plans directly...`, `renders only Sprint Dates and ignores outside allocation columns...`, and `formats dates, wraps long Task names...` | Component workflows verify the complete observable UI simplification and that header, Capacity, and Task cells are marked only when Member Daily Capacity is `0h`, not when Remaining Capacity is `0h`. |
...
| SPD-08 Documentation reconciliation | Documentation gate | `docs/project/architecture.md`, `docs/project/schedmind-context.md`, this evidence file, and `docs/project/README.md` | Static review only | Static review only |
...
- Sprint still stores only aggregate fields and Member/Task relations. Capacity,
  Project/WBS context, schedule, and allocation remain live projections.
- Add/Remove changes local reviewed membership only. It does not alter Task,
  Project, schedule, Assignee, allocation, or capacity data.
...
- Sprint Planning reuses Home's `WBSPanel`; it does not introduce a second Task
  ...
  selection untouched. Saving a Task regenerates local suggestion state but does
  not persist Sprint membership.
...
2. selected/candidate Task rows joined to Project and Assignee;
3. all WBS nodes for the represented Project IDs in one query, composed in memory
   into user-facing depth-first path/rank;
...
access. No schema change is required. PostgreSQL plan execution was not performed
...
netted variance, missing WBS path/order date, stale projection token, or altered
Sprint ownership behaviour.
...
order, one-grid Sprint-period allocation, Member Capacity-only presentation,
formatted dates, zero-capacity markers, Add/Remove behavior, focus preservation,
shared Task editing without navigation, post-save suggestion regeneration, and
mapped API fields.
...
4. Confirm there is no Sprint daily summary and each Member shows only period and
   per-Date Capacity plus Task allocation rows.
...
6. Confirm visible dates use `DD MMM YYYY`, long Task names wrap within `50ch`, and
   Task rows/picker omit WBS, aggregate allocation text, and First planned Date.
...
8. Add and remove a Task before Save; confirm Task rows change immediately while
   Member capacity remains unchanged and only Sprint Task membership is submitted.
...
10. Click a Task Name from Sprint Planning and confirm the same Edit Task dialog
    ...
    closes, Sprint Planning remains visible, suggestion is regenerated, and
    Sprint Task membership is still not persisted until Save Sprint Planning.
