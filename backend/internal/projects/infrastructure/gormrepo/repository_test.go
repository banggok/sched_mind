...
func TestRepositoryCreateListSearchAndPriority(t *testing.T) {
	...
	alpha, err := repository.CreateNext(ctx, "a", "Alpha", true, 20, now)
	if err != nil || alpha == nil {
		t.Fatalf("alpha: %#v %v", alpha, err)
	}
	beta, err := repository.CreateNext(ctx, "b", "Beta", true, 20, now)
	if err != nil || beta == nil || beta.Priority != 2 {
		t.Fatalf("beta: %#v %v", beta, err)
	}
	if _, err := repository.CreateNext(ctx, "duplicate", "ALPHA", true, 20, now); !errors.Is(err, domain.ErrNameExists) {
...
func TestRepositoryLifecycleDeleteAndRollback(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC()
	value, err := repository.CreateNext(ctx, "a", "Alpha", true, 20, now)
	...
	_, _ = repository.CreateNext(ctx, "b", "Beta", true, 20, now)
...
func TestRepositorySettingsPersistenceAndSchedulerRollback(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC()
	created, err := repository.CreateNext(ctx, "a", "Alpha", true, 20, now)
	if err != nil || created == nil || !created.AutomaticScheduling || created.ProjectBuffer != 20 {
		t.Fatalf("defaults: %#v %v", created, err)
	}
	updated, err := repository.UpdateSettings(ctx, "a", false, 35, now.Add(time.Hour), func(context.Context, string) error { t.Fatal("scheduler called while disabling"); return nil })
	if err != nil || updated == nil || updated.AutomaticScheduling || updated.ProjectBuffer != 35 {
		t.Fatalf("disable: %#v %v", updated, err)
	}
	_, err = repository.UpdateSettings(ctx, "a", true, 40, now.Add(2*time.Hour), func(context.Context, string) error { return errors.New("scheduler unavailable") })
	if err == nil {
		t.Fatal("expected scheduler failure")
	}
	stored, _ := repository.Find(ctx, "a")
	if stored == nil || stored.AutomaticScheduling || stored.ProjectBuffer != 35 {
		t.Fatalf("rollback: %#v", stored)
	}
	_, err = repository.UpdateDetails(ctx, "a", "Renamed", true, 40, now.Add(3*time.Hour), func(context.Context, string) error { return errors.New("scheduler unavailable") })
	if err == nil {
