...
> Details/Members form and page-level Task Review workspace, Sprint Member and Sprint Task membership, live Execution
> capacity/allocation projection, deterministic Task suggestion, overlap
> protection, Planned/Started metadata, drift warnings, and Sprint-specific
> persistence/API contracts. It does not own or change Project, WBS, scheduler,
> dependency, Actual Date, Actual Allocation, or capacity mutation behaviour.
>
...
**I want** to group scheduled Tasks and selected Members into a Sprint

**So that** I can review the planned work and capacity distribution for a fixed
calendar period without changing the authoritative schedule.
...
- Create and edit Sprint metadata through a two-step modal form:
  1. Sprint Details;
  2. Members.
- Manage Task Review on the main Sprint page after the Sprint exists.
...
- Display selected Tasks grouped by current Assignee, then by Project as level
  `0`, preserving the authoritative WBS ancestor structure down to each selected
  Task.
- Display current canonical Execution allocation per Date, including positive
  allocation outside the Sprint date range.
- Display in-Sprint capacity, in-Sprint allocation, remaining capacity or
  overcapacity, total Task Execution allocation, and outside-Sprint allocation.
- Add or remove Tasks manually in the page-level Task Review after Sprint
...
- Project Priority and WBS order are read only as deterministic Task suggestion
  tie-breakers; Sprint does not modify either value.
...
- A Member with no eligible Task is valid and remains visible in Task Review.
...
- At least one Member is required before Task Review or Save.
...
### 8.3 Main-page Task Review

After Create saves Details and Members, the main Sprint page opens the saved
Sprint's Task Review workspace. Task management is not rendered inside the
Create/Edit modal. The user may generate one deterministic suggestion from the
saved Sprint details/members.

Task Review provides:

- Member groups;
- within each Member, Project roots at level `0` and the authoritative WBS
  ancestor hierarchy down to selected executable Tasks;
- present that hierarchy through its indentation and names without displaying
  internal level labels or WBS ordering keys;
...
user to the page-level Task Review, where the user may explicitly regenerate.
...
### 9.2 Capacity Display

For every selected Member show:

- daily Sprint Execution Capacity;
- total Sprint Execution Capacity;
- selected In-Sprint Allocation;
- Remaining Capacity when allocation is below capacity;
- Overcapacity when allocation exceeds capacity.

```text
Remaining Capacity = max(0, Capacity - In-Sprint Allocation)
Overcapacity       = max(0, In-Sprint Allocation - Capacity)
```

At Sprint level show the same totals summed across selected Members.
...
- Zero capacity does not block Save.
- Overcapacity does not block Save and does not require confirmation.
- Overcapacity is never shifted to another Date and never becomes future debt.
...
- Render loading, empty, failure, and successful-add feedback inline beside the
  Task Review actions; opening the picker must not navigate or open another
  modal.
- Display Project, WBS, Task Name, Assignee, Execution Start, Execution End,
  In-Sprint Allocation, and Total Execution Allocation.
- Do not expose internal WBS ordering keys or numeric hierarchy-level labels in
  the picker; use user-facing Project/Task context and visual hierarchy instead.
...
Mandatory Tasks are ordered for display using:
...
Ordering never changes the must-include rule.
...
- In-Sprint Allocation;
- Total Execution Allocation;
- Outside-Sprint Allocation;
- Completed/Needs Review indicators when applicable.

All positive current Execution allocation Dates must remain discoverable even
when outside the Sprint Period.

### 12.3 Daily Allocation UI

- The default visible/focused range is the Sprint Period.
- Sprint boundary columns are visually identifiable.
- Positive allocation before Sprint Start or after Sprint End is available
  through the same horizontally scrollable/virtualized grid or an equivalent
  accessible outside-period disclosure.
- The UI must not silently truncate outside-period positive allocation.
- Non-working Dates may remain visible and show zero capacity/allocation.
- Daily values use hours in the repository-standard precision.
- Large date ranges require bounded rendering; do not permanently render an
  unbounded `Tasks × Dates` matrix.

### 12.4 Member Group Totals

Tasks are grouped by current Assignee when that Assignee is a selected Sprint
Member. Each group shows:

- Member Sprint Execution Capacity;
- summed selected In-Sprint Allocation;
- remaining capacity or overcapacity;
- summed Total Execution Allocation;
- daily capacity and daily selected allocation.

A selected Member with no Task remains as an empty group with capacity values.

### 12.5 Sprint Totals

Sprint totals distinguish:
...
Needs Review allocation must not be disguised as selected-member utilization
when its current Assignee is not a selected Member.
...
- Its readable allocation remains included in Needs Review and overall Sprint
  Task totals.
- Task entirely outside the Sprint shows `0h` In-Sprint Allocation and its full
  Total/Outside-Sprint Allocation; this condition alone may be informational
  rather than Needs Review when it remains otherwise eligible.
...
- Daily allocation cells expose Member, Task, full Date, capacity/allocation,
  and in/out-of-Sprint meaning to assistive technology.
- Overcapacity is not communicated by color alone.
...
- Responsive layouts may switch the daily grid to a date-row disclosure, but all
  required values and outside-Sprint allocation remain accessible.
...
6. Create uses modal Details and Members steps in order, then opens Task Review
...
19. Member and Sprint totals distinguish capacity, In-Sprint Allocation,
    remaining capacity, and overcapacity.
...
34. Task Review groups normal Tasks by current selected Assignee and renders
    each group as Project level `0` followed by its authoritative WBS hierarchy.
35. Selected Members with no Tasks remain visible with capacity totals.
36. Every Task displays Project, WBS, Name, Assignee, Execution dates, daily
    Execution allocation, In-Sprint, Outside-Sprint, and Total allocation.
37. All positive allocation outside the Sprint Period remains discoverable and
    is not silently truncated.
38. Utilization uses In-Sprint Allocation, not total Effort or Total Execution
    Allocation.
39. Large date ranges use bounded rendering without losing required allocation.
...
53. A Task shifted entirely outside Sprint remains visible with zero In-Sprint
    and complete outside/total allocation.
...
74. Rapid list/detail/suggestion changes cannot restore stale capacity,
    allocation, Member, or Task projection.
75. Backend read paths are set-based and avoid per-entity/per-Date N+1 queries.
76. All inputs/actions/daily values/warnings are keyboard and screen-reader
    accessible; overcapacity is not color-only.
77. Recoverable failures preserve valid modal input and page-level reviewed
    membership.
78. Every AC is supported by Code Inspection, Unit/Integration Test, and
...
- Daily allocation inside/outside Sprint and horizontal/disclosure accessibility.
...
3. Mandatory overdue Task ends before Sprint Start and appears with outside-only
   allocation.
4. Remaining capacity selects later Tasks until the last Task exceeds capacity.
5. Manually add a scheduled Task entirely after Sprint and view full allocation.
