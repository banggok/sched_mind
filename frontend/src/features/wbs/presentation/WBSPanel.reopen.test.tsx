...
describe("US-4.2 acceptance workflow through WBSPanel", () => {
  ...
  it("AC-2 AC-9 AC-13 does not offer Reopen for Closed, Grouping, or unfinished WBS", async () => {
    ...
    expect(within(detail).queryByRole("button", { name: "Reopen Task" })).toBeNull();
    ...
    await userEvent.click(within(groupItem).getByRole("button", { name: "View Group" }));
    const groupDialog = await screen.findByRole("dialog", { name: "Development" });
    expect(within(groupDialog).queryByRole("button", { name: "Reopen Task" })).toBeNull();
  ...
  it("AC-11 AC-14 preserves completed state after Forecast failure and permits retry", async () => {
    ...
    vi.mocked(api.reopen)
      .mockRejectedValueOnce(new Error("The Task could not be reopened. Try again."))
      .mockResolvedValueOnce(task("task", "Build API", undefined));
    renderPanel("open", values, api);
    let detail = await openTask("Build API");
    const confirmation = await openConfirmation(detail);
    await userEvent.click(within(confirmation).getByRole("button", { name: "Reopen Task" }));
    expect(await within(confirmation).findByText("The Task could not be reopened. Try again.")).toBeTruthy();
    expect(within(detail).getByText(/Completed on .*2026/)).toBeTruthy();

    await userEvent.click(within(confirmation).getByRole("button", { name: "Reopen Task" }));
    detail = await screen.findByRole("dialog", { name: "Edit Task" });
    await waitFor(() => expect(within(detail).queryByText(/Completed on/)).toBeNull());
  ...
  it("AC-15 AC-16 exposes alertdialog semantics without narrow-viewport overflow regressions", async () => {
    Object.defineProperty(window, "innerWidth", {
      configurable: true,
      value: 360,
    });
