...
describe('TeamMembersDashboardPage', () => {
  it('filters members and provides a clear no-results state', async () => {
    ...
    const gateway: TeamMembersGateway = {
      list: vi.fn().mockImplementation(async (query) => {
        const items = [member('andi', 'Andi'), member('budi', 'Budi')].filter(
          (item) => item.name.toLowerCase().startsWith(query.search.toLowerCase()),
        )
    ...
    expect(screen.getByRole('status', { name: 'Loading members' })).toBeTruthy()
    await waitFor(() =>
      expect(
  ...
  it('starts with no role selected and accepts a .5 capacity draft', async () => {
    ...
    const gateway: TeamMembersGateway = {
      list: vi.fn().mockResolvedValue({ items: [], page: 1, pageSize: 5, total: 0 }),
...
function emptyRolesGateway(): RolesGateway {
  return {
    list: vi.fn().mockResolvedValue({ items: [], page: 1, pageSize: 100, total: 0 }),
    create: vi.fn(),
    update: vi.fn(),
    delete: vi.fn(),
  }
